...
export default function TermsContent() {
  ...
  return (
    <div className="flex flex-col items-start p-0 gap-[80px] w-[1240px] mx-auto">
      <h1 className="w-[1240px] h-[65px] font-['DM_Sans'] font-bold text-[50px] leading-[130%] text-center tracking-[0.5px] text-[#272727]">
        정책 및 약관
      </h1>
      
      <div className="flex flex-col items-start p-0 gap-[30px] w-[1240px]">
        <div className="flex flex-row items-center p-0 gap-[10px] w-[651px] h-[44px]">
          <button 
            onClick={() => setActiveTab('terms')}
            className={`flex flex-row justify-center items-center px-[20px] py-[10px] gap-[10px] w-[202px] h-[44px] rounded-[50px] font-['DM_Sans'] font-normal text-[16px] leading-[24px] tracking-[0.5px] transition-colors ${
              activeTab === 'terms' 
                ? 'bg-[#298487] text-[#FCFCFC]' 
                : 'border border-[#272727] text-[#272727] hover:bg-[#F0F5F8]'
            }`}
          >
            서비스 이용약관
          </button>
          ...
          <button 
            ...
            className={`flex flex-row justify-center items-center px-[20px] py-[10px] gap-[10px] w-[263px] h-[44px] rounded-[50px] font-['DM_Sans'] font-normal text-[16px] leading-[24px] tracking-[0.5px] transition-colors ${
              activeTab === 'privacy' 
                ? 'bg-[#298487] text-[#FCFCFC]' 
                : 'border border-[#272727] text-[#272727] hover:bg-[#F0F5F8]'
            }`}
          ...
          <button 
            ...
            className={`flex flex-row justify-center items-center px-[20px] py-[10px] gap-[10px] w-[166px] h-[44px] rounded-[50px] font-['DM_Sans'] font-normal text-[16px] leading-[24px] tracking-[0.5px] transition-colors ${
              activeTab === 'marketing' 
                ? 'bg-[#298487] text-[#FCFCFC]' 
                : 'border border-[#272727] text-[#272727] hover:bg-[#F0F5F8]'
            }`}
        ...
        <div className="flex flex-col items-start p-0 gap-[30px] w-[1240px]">
          ...
          <div className="w-[1240px] font-['DM_Sans'] font-normal text-[16px] leading-[160%] text-[#272727] whitespace-pre-wrap">
            {activeTab === 'terms' && getTermsContent()}
            {activeTab === 'privacy' && getPrivacyContent()}
            {activeTab === 'marketing' && getMarketingContent()}
          </div>
...
function getTermsContent() {
  return `제1조(목적)
이 약관은 퀀텀리포트 주식회사 회사(전자상거래 사업자)가 운영하는 퀀텀리포트 주식회사 사이버 몰(이하 “몰”이라 한다)에서 제공하는 인터넷 관련 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리,의무 및 책임사항을 규정함을 목적으로 합니다. ※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」

제2조(정의)
 1. “몰” 이란 퀀텀리포트 주식회사 회사가 재화 또는 용역(이하 “재화등”이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
1. “몰”은 이 약관의 내용과 상호 및 대표자 성명, 영업소 소재지 주소(소비자의 불만을 처리할 수 있는 곳의 주소를 포함), 전화번호?모사전송번호?전자우편주소, 사업자등록번호, 통신판매업신고번호, 개인정보관리책임자등을 이용자가 쉽게 알 수 있도록 00 사이버몰의 초기 서비스화면(전면)에 게시합니다. 다만, 약관의 내용은 이용자가 연결화면을 통하여 볼 수 있도록 할 수 있습니다.
...
3. “몰”은 전자상거래등에서의 소비자보호에관한법률, 약관의규제에관한법률, 전자거래기본법, 전자서명법, 정보통신망이용촉진등에관한법률, 방문판매등에관한법률, 소비자보호법 등 관련법을 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.
...
제4조(서비스의 제공 및 변경)
 1. “몰”은 다음과 같은 업무를 수행합니다.
    a. 재화 또는 용역에 대한 정보 제공 및 구매계약의 체결
    b. 구매계약이 체결된 재화 또는 용역의 배송
    c. 기타 “몰”이 정하는 업무
...
3. 사업종목의 전환, 사업의 포기, 업체간의 통합 등의 이유로 서비스를 제공할 수 없게 되는 경우에는 “몰”은 제8조에 정한 방법으로 이용자에게 통지하고 당초 “몰”에서 제시한 조건에 따라 소비자에게 보상합니다. 다만, “몰”이 보상기준 등을 고지하지 아니한 경우에는 이용자들의 마일리지 또는 e-Money 등을 “몰”에서 통용되는 통화가치에 상응하는 현물 또는 현금으로 이용자에게 지급합니다.
...
2. “몰”은 제1항과 같이 회원으로 가입할 것을 신청한 이용자 중 다음 각호에 해당하지 않는 한 회원으로 등록합니다.
    a. 가입신청자가 이 약관 제7조제3항에 의하여 이전에 회원자격을 상실한 적이 있는 경우, 다만 제7조제3항에 의한 회원자격 상실후 3년이 경과한 자로서 “몰”의 회원재가입 승낙을 얻은 경우에는 예외로 한다.
    b. 등록 내용에 허위, 기재누락, 오기가 있는 경우
    c. 기타 회원으로 등록하는 것이 “몰”의 기술상 현저히 지장이 있다고 판단되는 경우
...
2. 회원이 다음 각호의 사유에 해당하는 경우, “몰”은 회원자격을 제한 및 정지시킬 수 있습니다.
    a. 가입신청시에 허위내용을 등록한 경우
    b. “몰”을 이용하여 구입한 재화등의 대금, 기타 “몰”이용에 관련하여 회원이 부담하는 채무를 기일에 지급하지 않는 경우
    c. 다른사람의 “몰” 이용을 방해하거나 그 정보를 도용하는 등 전자상거래 질서를 위협하는 경우
    d. “몰”을 이용하여 법령 또는 이 약관이 금지하거나 공서양속에 반하는 행위를 하는 경우
3. “몰”이 회원 자격을 제한?정지 시킨후, 동일한 행위가 2회이상 반복되거나 30일이내에 그 사유가 시정되지 아니하는 경우 “몰”은 회원자격을 상실시킬 수 있습니다.
4. “몰”이 회원자격을 상실시키는 경우에는 회원등록을 말소합니다. 이 경우 회원에게 이를 통지하고, 회원등록 말소전에 최소한 30일 이상의 기간을 정하여 소명할 기회
...
1. “몰”이용자는 “몰”상에서 다음 또는 이와 유사한 방법에 의하여 구매를 신청하며, “몰”은 이용자가 구매신청을 함에 있어서 다음의 각 내용을 알기 쉽게 제공하여야 합니다. 단, 회원인 경우 제2호 내지 제4호의 적용을 제외할 수 있습니다.
    a. 재화등의 검색 및 선택
    b. 성명, 주소, 전화번호, 전자우편주소(또는 이동전화번호) 등의 입력
    c. 약관내용, 청약철회권이 제한되는 서비스, 배송료?설치비 등의 비용부담과 관련한 내용에 대한 확인
    d. 이 약관에 동의하고 위 3.호의 사항을 확인하거나 거부하는 표시(예, 마우스 클릭)
    e. 재화등의 구매신청 및 이에 관한 확인 또는 “몰”의 확인에 대한 동의
    f. 결제방법의 선택
...
1. “몰”은 제9조와 같은 구매신청에 대하여 다음 각호에 해당하면 승낙하지 않을 수 있습니다. 다만, 미성년자와 계약을 체결하는 경우에는 법정대리인의 동의를 얻지 못하면 미성년자 본인 또는 법정대리인이 계약을 취소할 수 있다는 내용을 고지하여야 합니다.
    ...
    b. 미성년자가 담배, 주류등 청소년보호법에서 금지하는 재화 및 용역을 구매하는 경우
    c. 기타 구매신청에 승낙하는 것이 “몰” 기술상 현저히 지장이 있다고 판단하는 경우
...
제11조(지급방법)
1. “몰”에서 구매한 재화 또는 용역에 대한 대금지급방법은 다음 각호의 방법중 가용한 방법으로 할 수 있습니다. 단, “몰”은 이용자의 지급방법에 대하여 재화 등의 대금에 어떠한 명목의 수수료도 추가하여 징수할 수 없습니다.
    a. 폰뱅킹, 인터넷뱅킹, 메일 뱅킹 등의 각종 계좌이체
    b. 선불카드, 직불카드, 신용카드 등의 각종 카드 결제
    c. 온라인무통장입금
    d. 전자화폐에 의한 결제
    e. 수령시 대금지급
    f. 마일리지 등 “몰”이 지급한 포인트에 의한 결제
    g. “몰”과 계약을 맺었거나 “몰”이 인정한 상품권에 의한 결제
...
제14조(환급)
1. “몰”은 이용자가 구매신청한 재화등이 품절 등의 사유로 인도 또는 제공을 할 수 없을 때에는 지체없이 그 사유를 이용자에게 통지하고 사전에 재화 등의 대금을 받은 경우에는 대금을 받은 날부터 2영업일 이내에 환급하거나 환급에 필요한 조치를 취합니다.
...
2. 이용자는 재화등을 배송받은 경우 다음 각호의 1에 해당하는 경우에는 반품 및 교환을 할 수 없습니다.
    ...
    c. 시간의 경과에 의하여 재판매가 곤란할 정도로 재화등의 가치가 현저히 감소한 경우
    d. 같은 성능을 지닌 재화등으로 복제가 가능한 경우 그 원본인 재화 등의 포장을 훼손한 경우
...
퀀텀리포트는 다음의 목적을 위하여 개인정보를 처리합니다. 처리하고 있는 개인정보는 다음의 목적 이외의 용도로는 이용되지 않으며, 이용 목적이 변경되는 경우에는 개인정보 보호법 제18조에 따라 별도의 동의를 받는 등 필요한 조치를 이행할 예정입니다.
...
제1조 (마케팅 정보 수신 동의)
퀀텀리포트는 회원님께 다양한 서비스 및 이벤트 정보를 제공하기 위해 마케팅 목적의 개인정보를 수집·이용하고자 합니다.
